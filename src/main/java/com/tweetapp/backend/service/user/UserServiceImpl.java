...
public class UserServiceImpl implements UserService {
	...
	if (Objects.isNull(viewUserRequest)) {
	    return ViewUserResponse.builder()
		    ...
		    ._message(INVALID_ARGUMENT + viewUserRequest)
		    .build();
	}
	...
	if (Objects.isNull(mail)) {
	    return ViewUserResponse.builder()
		    ._status_code(STATUS_FAILED)
		    ._message(PROVIDED_EMAIL_IS_NULL)
		    .build();
	}
	...
	if (optionalUser.isPresent()) {
	    ...
	    return ViewUserResponse.builder()
		    ._status_code(STATUS_SUCCESS)
		    ._message(USER_RECORD_FOUND)
	    ...
	    return ViewUserResponse.builder()
		    ._status_code(STATUS_FAILED)
		    ._message(NO_USER_FOUND_WITH_THIS_EMAIL + mail)
		    .build();
	}
    }
	...
	if (Objects.isNull(createUserRequest)) {
	    return CreateUserResponse.builder()
		    ._status_code(STATUS_FAILED)
		    ._message(INVALID_ARGUMENT + createUserRequest)
		    .build();
	}
	...
	try {
	    ...
	    createUserResponse = CreateUserResponse.builder()
            	    	._status_code(STATUS_SUCCESS)
            	    	._message(USER_RECORD_CREATED)
            	    	.firstName(savedUser.getFirstName())
            	    	.lastName(savedUser.getLastName())
            	    	.email(savedUser.getEmail())
            	    	.dateOfJoin(savedUser.getDateOfJoin())
            	    	.build();
	} catch (Exception e) {
	    createUserResponse = CreateUserResponse.builder()
        	    	._status_code(STATUS_FAILED)
        	    	._message(USER_RECORD_CANNOT_BE_CREATED + ExceptionUtils.getMessage(e))
        	    	.build();
	}
	...
	if (Objects.isNull(updateUserRequest)) {
	    return UpdateUserResponse.builder()
		    ._status_code(STATUS_FAILED)
		    ._message(INVALID_ARGUMENT + updateUserRequest)
		    .build();
	}
	...
	if (foundByEmail.isPresent()) { // When 'email' present -- then can update
		...
		return UpdateUserResponse.builder()
			._status_code(STATUS_FAILED)
			._message(USER_RECORD_NOT_UPDATED)
			.email(email)
			.build();
	    }
	...
	if (Objects.isNull(loginRequest)) {
	    return LoginResponse.builder()
			._status_code(STATUS_FAILED)
			._message(INVALID_LOGIN_REQUEST)
			.build();
	}
	...
	if (Objects.isNull(encodedPassword) || Objects.isNull(email)) {
	    return LoginResponse.builder()
		    ._status_code(STATUS_FAILED)._message(
			    (Objects.isNull(encodedPassword) ? PASSWORD_CAN_NOT_BE_EMPTY_OR_NULL : "")
		...
		if (StringUtils.equals(password, encodedPassword)) {
		    return LoginResponse.builder()
			    ._status_code(STATUS_SUCCESS)._message("Credentials are matched").build();
		} else {
		    return LoginResponse.builder()
			    ._status_code(STATUS_FAILED)._message("Credentials are not matched").build();
		}
	    } else {
		return LoginResponse.builder()
